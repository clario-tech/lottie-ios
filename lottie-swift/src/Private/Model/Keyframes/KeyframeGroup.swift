...
extension KeyframeGroup where T == Vector1D {
    func flip() -> KeyframeGroup<Vector1D> {
        KeyframeGroup(keyframes: keyframes.map{ Keyframe(value: Vector1D(-$0.value.value), time: Double($0.time), isHold: $0.isHold, inTangent: $0.inTangent, outTangent: $0.outTangent, spatialInTangent: $0.spatialInTangent, spatialOutTangent: $0.spatialOutTangent) })
    }
}
